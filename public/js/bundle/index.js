...
if (userDataForm) userDataForm.addEventListener("submit", (e)=>{
    ...
    const email = document.getElementById("email").value;
    (0, _changeAccount.userChange)(email, name);
});
// add event listener to the .form-user-data element
...
/* eslint-disable */ var _alert = require("./alert");
const userChange = async (email, name)=>{
    // Send a POST request
...
const passwordChange = async (currentPassword, newPassword, newPasswordConfirm)=>{
    // Send a POST request
    const request = await fetch("http://localhost:3000/api/v1/users/updatePassword", {
        method: "PATCH",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json"
